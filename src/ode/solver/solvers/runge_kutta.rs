    ...
    V: Value + MulAssign<T>,
{
    ...
    fn step1(&mut self, h: T) {
        ...
        rest[0] += &points[0];
        rest[0] += &*slope;
        (self.yp)(&mut self.slopes[1], &mut rest[0]);
    }
    ...
    fn step2(&mut self, h: T) {
        ...
        rest[0] += &points[0];
        rest[0] += &*slope;
        (self.yp)(&mut self.slopes[2], &mut rest[0]);
    }
    ...
    fn step3(&mut self, h: T) {
        ...
        rest[0] += &points[0];
        rest[0] += &*slope;
        (self.yp)(&mut self.slopes[3], &mut rest[0]);
    }
}

impl<'a, T, V> SsOdeSolver<T, V> for RungeKutta<'a, T, V>
where
    T: Time,
    V: Value + MulAssign<T>,
{
