...
fn new_sparse() {
    ...
    fn with_smatrix() {
        let result = SMatrix::<f32, M, N>::new_sparse();
        assert_eq!(result.size(), (M, N));
    ...
    fn with_dmatrix() {
        let result = DMatrix::<f32>::new_sparse((M, N));
        assert_eq!(result.size(), (M, N));
        assert_eq!(result.is_sparse(), true);
        assert!(result.is_zero());
    }
...
fn make() {
    ...
    fn with_svector() {
        let result = SVector::<f32, N>::make();
        let matrix = result.build();
        assert_eq!(matrix.size(), (N, 1));
    }

    fn with_dvector() {
        let result = DVector::<f32>::make(N);
        let matrix = result.build();
        assert_eq!(matrix.size(), (N, 1));
    }

    fn with_smatrix() {
        let result = SMatrix::<f32, M, N>::make();
        let matrix = result.build();
        assert_eq!(matrix.size(), (M, N));
    }

    fn with_dmatrix() {
        let result = DMatrix::<f32>::make((M, N));
        let matrix = result.build();
        assert_eq!(matrix.size(), (M, N));
    }
}
...
fn index() {
    ...
    fn with_vector() {
        let pos = M / 2;
        let val = 3.0;
        let target = DVector::make(N).values([(pos, val)]).build();
        let result = target[pos];
        assert_eq!(result, val);
    }
}
